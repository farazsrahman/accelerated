...
The couse will study gradient descent in various problem settings, along with how choices of $\{\alpha_t\}$ can affect convergence rates, numerical robustness, and \TODO{add desc.}
% \\\\
...
\section{Foundational Questions about GD}

\begin{enumerate}[label=Q\arabic*)]
\item Why move in the direction $-\nabla f$
\item Can GD converge? (with any step size)
\item How fast? (optimal step size)
\end{enumerate}

\subsection{Why move in the direction $-\nabla f$}
...
\item \textbf{Roots:} \[\cb{\cos\p{\frac{2t+1}{2n}\pi}}_{t=0 \dots n-1}\]
	...
	\item \includegraphics[width=400pt]{Pasted image 20251027222540.png}
	\end{itemize}
...
A core message to takeaway here is that there are many equivalent ways to derive step-size schedules yielding accelerated descent \textit{and} they all correspond to some way to intuit the Chebyshev polynomials. We can see this because to pick a "next" step size we need to know the "next" root of the $n$ step polynomial. 
...
\end{align*}
\includegraphics{Screenshot 2025-10-27 at 8.58.00 PM.png}
...
\subsection{Random Stepsizes}

Suppose $\alpha_t \sim \mu$ iid. What is the optimal $\mu$ to sample from?

Informally our rate is \[\max_{f, \bx_0 = \bx^*} \frac{\norm{\bx_n-\bx^*}}{\norm{\bx_0 - \bx^*}}\]
Suppose $\alpha_t \sim \mu$ iid. What is the optimal $\mu$ to sample from? What is the corresponding rate?
\[R(\mu) = \max_{f, \bx_0} \lim_{n\rightarrow \infty} \E\b{\frac{\norm{\bx_n - \bx^*}}{\norm{\bx_0-\bx^*}}^{1/n}}\]

Observations: rate is invariant w.r.t. ordering of the step-sizes $\cb{\alpha_t}$
\begin{enumerate}
\item Optimal deterministic step sizes are the inverse roots of Chebyshev polynomials
\item Convergence rates depends only on $\hat \mu_n = \frac{1}{n}\sum\delta(\alpha_t)$ (i.e. uniform on the roots)
\item Intuitively the transform of the uniform on the unit circle is 
\end{enumerate}


\subsection{Solving the Extremal Problem via Potential Theory:}
...
\includegraphics[width=400pt]{Screenshot 2025-10-30 at 4.43.11 PM.png}
